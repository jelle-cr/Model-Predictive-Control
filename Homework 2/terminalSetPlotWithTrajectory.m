function terminalSetPlotWithTrajectory(font,X_set,Feasibleset_x0,INV_set,x1,x2,x3,x4,xbar)
    s = get(0, 'ScreenSize');
...
%     plot(x1(1,length(x1)),x1(2,length(x1)),'^','Color','#fcb103',MarkerSize=8,LineWidth=2);
    ...
    plot(xbar(1,:),xbar(2,:),'o','Color','black',LineWidth=2);
    plot(xbar(1,1),xbar(2,1),'square','Color','black',MarkerSize=8,LineWidth=2);
    plot(xbar(1,length(xbar)),xbar(2,length(xbar)),'^','Color','black',MarkerSize=8,LineWidth=2);
    ax = gca;
    ax.FontSize=font-6;
